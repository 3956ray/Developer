...
class SenseVoiceEvaluationTest {
    ...
    @Test fun actualEngineOfflineSilenceAndRelease() {
        ...
        SenseVoiceCandidate(InstrumentationRegistry.getInstrumentation().context,context).use { engine ->
            engine.prepare()
    ...
    @Test fun syntheticMemoryFixtureServer() {
        ...
        assertFalse(context.packageManager.getPackageInfo(context.packageName,android.content.pm.PackageManager.GET_PERMISSIONS).requestedPermissions.orEmpty().contains(android.Manifest.permission.INTERNET))
        SenseVoiceCandidate(InstrumentationRegistry.getInstrumentation().context,context).use { engine ->
            ...
            context.getFileStreamPath("sensevoice-load.json").writeText(JSONObject().put("coldLoadMs",loadMs).put("totalPssKiB",memory.totalPss).put("nativeAllocatedBytes",android.os.Debug.getNativeHeapAllocatedSize()).toString(2))
            LocalServerSocket("thinkv2-sensevoice-fixture").use { server ->
                server.accept().use { socket ->
                    ...
                    repeat(cases) {
                        ...
                        require(metadata.optString("mode")=="throughput")
                        val record=run {
                        ...
                        val text=try { engine.decoder().use { decoder ->
                            while(remaining>0) {
                        ...
                        context.getFileStreamPath("sensevoice-synthetic-results.json").writeText(output.toString(2))
