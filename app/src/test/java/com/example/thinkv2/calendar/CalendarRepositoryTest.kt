...
class CalendarRepositoryTest {
    ...
    @Test fun schemaFourMigrationPreservesAllExistingTablesAndOriginalFieldNulls()=Store(temp.newFile()).use { s ->
        ...
        s.sql.execute("DROP TABLE calendar_imports");s.sql.execute("PRAGMA user_version=4");s.notes.initialize()
        before.forEach { (table,rows) ->assertEquals(rows,s.sql.query("SELECT * FROM $table ORDER BY 1")) };assertEquals("5",s.sql.query("PRAGMA user_version").single().single())
