...
class CalendarRepositoryTest {
    ...
    @Test fun schemaFourMigrationPreservesAllExistingTablesAndOriginalFieldNulls()=Store(temp.newFile()).use { s ->
        ...
        s.sql.execute("DROP TABLE calendar_imports");s.sql.execute("DROP TABLE ai_acceptances");s.sql.execute("DROP TABLE note_relations");s.sql.execute("DROP TABLE correction_vocabulary");s.sql.execute("PRAGMA user_version=4");s.notes.initialize()
        before.forEach { (table,rows) ->assertEquals(rows,s.sql.query("SELECT * FROM $table ORDER BY 1")) };assertEquals("8",s.sql.query("PRAGMA user_version").single().single())
