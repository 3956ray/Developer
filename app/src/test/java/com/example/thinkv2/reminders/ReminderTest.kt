...
class ReminderTest {
    ...
    @Test fun schemaTwoUpgradePreservesNotesDraftsCategoriesAndRollsBackOnFailure() {
        ...
        for(table in listOf("reminders","reminder_events","reminder_runtime")) sql.execute("DROP TABLE $table")
        ...
        assertEquals(listOf(listOf("3")),sql.query("PRAGMA user_version"));notes.close()
    }
