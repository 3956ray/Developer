...
class ReminderTest {
    ...
    @Test fun schemaTwoUpgradePreservesNotesDraftsCategoriesAndRollsBackOnFailure() {
        ...
        for(table in listOf("reminders","reminder_events","reminder_runtime","backup_imports","backup_origins")) sql.execute("DROP TABLE $table")
        ...
        assertEquals(listOf(listOf("4")),sql.query("PRAGMA user_version"));notes.close()
    }
