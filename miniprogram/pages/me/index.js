...
const DEMO = Boolean(api.config.demo && api.config.demo.enabled === true);
const DEMO_PURPOSE = '演示数据／演示身份：使用维护者发行的300秒一次性演示凭证建立本应用会话，不代表微信验证或真实会员资格。登录不授予馆方权限。凭证一经尝试不可重用，结果未确认需领取新凭证。可拒绝并继续浏览场馆。';
Page({
  data: { status: '未登录', detail: '登录后可使用个人服务；会员身份需馆方现场核验。', authenticated: false, busy: false,
    configured: api.configured(), demo: DEMO, ticketInput: '', testing: api.config.environment === 'test', privacy: DEMO ? DEMO_PURPOSE : PURPOSE, showPrivacy: false },
  ...
  onHide() { this.clearDemoCode(); this._visible = false; this._generation++; this._queuedInteraction = null; this.stopTimer(); },
  onUnload() { this.clearDemoCode(); this._visible = false; this._generation++; this.stopTimer(); },
  ...
  async acceptSession(response) {
    ...
    if (DEMO && (!s || s.gymId !== api.config.gymId || s.identityMode !== 'test' || s.demoEnabled !== true || s.simulation !== true)) throw { code: 'SCOPE_MISMATCH' };
    ...
    this.setData({ authenticated: true, status: s.simulation ? (DEMO ? '演示身份 · 已登录' : '模拟身份 · 已登录') : '已登录', detail: '仅建立本应用身份；会员资格和馆方权限需另外核验。' });
  ...
  showError(error) {
    if (error.code === 'SCOPE_MISMATCH') { this.forget(); this.clearDemoCode(); this.setData({ configured: false, status: '服务模式不匹配', detail: '已清除会话并关闭演示操作，请核对服务门店与身份配置。' }); }
    else if (error.status === 401) { this.forget(); this.setData({ status: '登录已失效', detail: '请主动重新登录，场馆浏览仍可使用。' }); }
    else this.setData({ authenticated: false, status: '暂时无法确认', detail: error.code === 'NOT_CONFIGURED' ? '服务尚未配置。' : DEMO ? '请核对连接并领取新的演示凭证后主动重试；不会自动登录。' : '请检查连接后重试；不会自动重新登录。' });
  },
  ...
  demoCodeInput(event) { this._demoCode = event.detail.value; },
  clearDemoCode() { this._demoCode = ''; this.setData({ ticketInput: '' }); },
  login() {
    if (this.data.busy || !this.data.configured || !api.configured()) return;
    const confirmedGeneration=this._generation;
    wx.showModal({ title: DEMO ? '演示登录用途说明' : '登录用途说明', content: DEMO ? DEMO_PURPOSE : PURPOSE, confirmText: '同意登录', cancelText: '暂不登录',
      success: async result => {
        if(!this._visible||confirmedGeneration!==this._generation)return;
        if (!result.confirm) { this.clearDemoCode(); this.setData({ detail: '已拒绝本次登录，你仍可浏览场馆。' }); return; }
        if (this.data.busy) return;
        this.setData({ busy: true }); const generation = ++this._generation;
        try {
          let code;
          if (DEMO) {
            code = this._demoCode; this.clearDemoCode();
            if (typeof code !== 'string' || !/^[A-Za-z0-9_-]{43}$/.test(code)) throw { code: 'DEMO_CREDENTIAL_REQUIRED' };
            await api.request('/health', 'GET');
          } else code = await api.loginCode();
