...
for case in (plan['cases'] if mode=='recognition' else plan['cases'][:10]):
 ...
 audio[case['id']]=pcm;metadata[case['id']]={'voice':plan['voice'],'rate':plan['rate'],'host':platform.mac_ver()[0],'generatorSha256':hashlib.sha256(pathlib.Path('/private/tmp/thinkv2-controls-synthetic').read_bytes()).hexdigest(),'seconds':len(pcm)/32000}
 assert sum(map(len,audio.values()))<64*1024*1024
 print('generated',case['id'],len(pcm)/32000,flush=True)
 del r
cases=[]
if mode=='recognition':
 for case in plan['cases']:cases.append((dict(case,mode='realtime',generator=metadata[case['id']]),audio[case['id']]))
else:
...
def recv_exact(s,n):
 ...
 return result
results=[]
with socket.create_connection(('127.0.0.1',5053),timeout=180) as s:
