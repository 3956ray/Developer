...
test('CP5 production cleanup wiring runs all domains at startup/hourly, preserves controls and retries failed retention',async t=>{
 ...
 x.db.prepare('INSERT INTO schedule_snapshots(id,publication_revision,time_zone,coverage_json,coverage_end_at,courses_json,published_at,retired_at) VALUES(?,?,?,?,?,?,?,?)').run(randomUUID(),999,'Asia/Taipei','{}',START-31*DAY,'[]',START-31*DAY,START-30*DAY);
