...
test('C01/D01 bounded cleanup rollback/restart preserves current snapshot/draft/withdrawn; published timezone frozen',async t=>{
 const x=await setup(t);save(x,[]);publish(x);const snapshot=x.db.prepare('SELECT * FROM schedule_snapshots').get();const insert=x.db.prepare('INSERT INTO schedule_snapshots(id,publication_revision,time_zone,coverage_json,coverage_end_at,courses_json,published_at,retired_at) VALUES(?,?,?,?,?,?,?,?)');for(let n=2;n<207;n++)insert.run(randomUUID(),n,snapshot.time_zone,snapshot.coverage_json,snapshot.coverage_end_at,snapshot.courses_json,START,START);
