...
test('durable probe survives full process restart and abrupt termination', async t => {
  ...
  assert.equal(health.data.migrations, 4); t.diagnostic(`health: ${JSON.stringify(health)}`);
...
test('failed new migration rolls back DDL and ledger atomically', t => {
  ...
  writeFileSync(resolve(folder, '005-broken.sql'), 'CREATE TABLE rollback_marker (id INTEGER); INSERT INTO missing_table VALUES (1);');
  ...
  assert.equal(db.prepare('SELECT count(*) n FROM schema_migrations').get().n, 4); db.close();
});
